...
{
	usb_init_urb(urb);
	usb_fill_iso_urb(urb, rt->chip->dev,
			 usb_sndintpipe(rt->chip->dev, COMM_EP),
			 buffer, 0, handler, context, 1, 0, 0);
}
...
int usb6fire_comm_init(struct sfire_chip *chip)
{
	...
	usb_fill_iso_urb(urb, chip->dev, usb_rcvintpipe(chip->dev, COMM_EP),
			 rt->receiver_buffer, COMM_RECEIVER_BUFSIZE,
			 usb6fire_comm_receiver_handler, rt, 1, 0, 0);
